...
Future<List<Request>> getRequests() async {
  ...
  await connections.get().then((value) => {
...
Future<bool> acceptRequest(String to, String from) async {
  var connections1 =
      FirebaseFirestore.instance.collection('connections').doc(to);
  await connections1.collection('Friends').add({'email': from});
  var connections2 =
      FirebaseFirestore.instance.collection('connections').doc(from);
  await connections2.collection('Friends').add({'email': to});
  return true;
}
...
Future<bool> closeAwaiting(id) async {
  ...
  var closeAwaiting =
      FirebaseFirestore.instance.collection('awaitingConnections');
  await closeAwaiting.doc(id).delete().then((e) => {print('suc')});
  return toReturn;
}
...
Future<List<Plan>> getPlans() async {
  List<Plan> userPlans = [];
  var plans = FirebaseFirestore.instance
      .collection('plans')
      .doc(user!.email)
      .collection('plan');
        ...
        value.docs.forEach((element) {
          if (!element['isDaily']) {
            ...
            for (int i = 0; i < days.length; i++) {
              DateTime toDelete = DateTime.fromMicrosecondsSinceEpoch(
                  deleteAfter[i].microsecondsSinceEpoch);
              if (toDelete.isBefore(DateTime.now())) {
                deleteAfter.remove(deleteAfter[i]);
                days.remove(days[i]);
              }
            }
            if (deleteAfter.length != element['deleteAfter'] &&
                deleteAfter.length > 0) {
              plans
                  .doc(element.id)
                  .update({'deleteAfter': deleteAfter, 'days': days});
          ...
          if (element['isDaily'] == false) {
            userPlans.add(Plan(
                ...
                element['isDaily'],
                element['days'],
                element['noRepeatDays']));
          } else {
            print(element.id.toString());
            userPlans.add(Plan.withAccept(
                element.id,
                element['title'],
                element['desc'],
                element['time'],
...
Future<void> insertPlan(title, details, date, TimeOfDay time, repeat, isDate,
    [repeatDays = 0]) async {
  ...
  var androidDetails = new AndroidNotificationDetails("plan", "Plan is Due",
      channelDescription: "Your Plan", importance: Importance.high);
  var generalNotifDetails = new NotificationDetails(android: androidDetails);
  var scheduledTime;
  if (!isDate) {
    List<String> days = [];
    ...
    if (!repeat && !isDate) {
      var plans =
          FirebaseFirestore.instance.collection('plans').doc(user!.email);
      await plans.collection('plan').add({
        ...
        weekly.forEach((time1) async {
          await flutterLocalNotificationsPlugin.zonedSchedule(
              value.id.hashCode + time1.hashCode,
              'Reminder: ' + title,
              details,
              time1,
              generalNotifDetails,
      ...
      await plans.collection('plan').add({
        ...
        weekly.forEach((time1) async {
          ...
          for (int i = 0; i < repeatDays; i++) {
            await flutterLocalNotificationsPlugin.zonedSchedule(
                value.id.hashCode + time1.hashCode + i.hashCode,
                'Reminder: ' + title,
                details,
                myTime,
                generalNotifDetails,
    ...
    plans.collection('plan').add({
      ...
      await flutterLocalNotificationsPlugin.zonedSchedule(value.id.hashCode,
          'Reminder: ' + title, details, scheduledTime, generalNotifDetails,
          uiLocalNotificationDateInterpretation:
              UILocalNotificationDateInterpretation.absoluteTime,
          androidAllowWhileIdle: true);
      userName.forEach((element) async {
        if (element != user!.email) {
          var userPlan =
              FirebaseFirestore.instance.collection('plans').doc(element);
          ...
          await userToken.get().then((value) {
          ...
          sendNotification('You were tagged in a Plan', 'By ${user!.email}',
              [reqUserToken], 'msg_tag');
        }
      });
    });
