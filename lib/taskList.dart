 ...
 void checkConnection() async {
   try {
     ...
     showDialog(context: context, builder: (builder){
       return AlertDialog(title: Text('This app cannot work properly without Internet'),actions: [
  ...
  getPlans().then((value) => {
    ...
    flutterLocalNotificationsPlugin.initialize(initializationSettings, onSelectNotification: (notifSelected)async{
          ...
          return Container(
              ...
              child: Padding(
                ...
                child: ListTile(
                  ...
                  trailing:
                  !userPlans[index].isDate?
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      userPlans[index].repeat? IconButton(onPressed: (){}, icon: Icon(CupertinoIcons.repeat,size: 20, color: Colors.indigo,))
                      ...
                      Material(
                        ...
                        IconButton(onPressed: (){
                          showDialog(context: context, builder: (BuildContext context){
                            var x = userPlans[index].repeat? '. It was set to repeat for '+userPlans[index].numRepDays.toString() + ' weeks.':'.';
                            return new AlertDialog(
                                content: Text('This Plan is Scheduled for '+userPlans[index].days.join(', ') + ' at ' + DateFormat.jm().format(userPlans[index].time).replaceAll(' ', '') + x));
                          });
                        }, icon: Icon(FontAwesomeIcons.infoCircle),iconSize: 20,color: Colors.indigo, ),)
                    ],
                  ):
                  // Text(
                  //
                  ...
                  CircularProgressIndicator(strokeWidth: 1,)
                    ...
                    IconButton(onPressed: () async{
                     ...
                     await   plans.doc(userPlans[index].id).update({
                          'accept':true,
                        });
                      ...
                      var scheduledTime = tz.TZDateTime.from(userPlans[index].time, loc);

                      await flutterLocalNotificationsPlugin.zonedSchedule(
                          userPlans[index].id.hashCode, 'Reminder: ' + userPlans[index].title, userPlans[index].details,
                          scheduledTime,
                          generalNotifDetails,
                         ...
                         IconButton(onPressed: (){
                           showDialog(context: context, builder: (context){
                             ...
                             return AlertDialog(
                               ...
                               Text(userPlans[index].people.join(', ').replaceAll('@gmail.com', '') + ' are included in this Plan.'),


                             );
                           });
                         }, icon: Icon(FontAwesomeIcons.userFriends,),iconSize: 20,color: Colors.indigo,),),
                         ...
                         IconButton(onPressed: (){
                           showDialog(context: context, builder: (context){
                             ...
                             return AlertDialog(
                               ...
                               Text('This Plan is Scheduled for '+formatter.format(userPlans[index].time)+' at ' +DateFormat.jm().format(userPlans[index].time).replaceAll(' ', '') + '.'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: PreferredSize(
          ...
          child: AppBar(
            leading: IconButton(icon: Badge(badgeContent: Text(''),showBadge: showBadge , child: Icon(CupertinoIcons.bars), position: BadgePosition.bottomEnd(),), onPressed: ()=> scaffoldKey.currentState!.openDrawer(),),
